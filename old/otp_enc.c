...
void ConnectToServer(char *portString, char *plainTextFileName, char *keyFileName)
{
	...
	// Combine the two plaintext and key and send both
	// Create a temp file

	SendFileToServer(sockfd, plainTextFileName); // Send the plaintext file
	// SendFileToServer(sockfd, keyFileName); // Send the key file
...
void SendFileToServer(int sockfd, char *fileName)
{
	...
	bzero(buffer, 256);
	strncpy(buffer, fileSizeString, 255);
	...
	// printf("otp_enc: waiting for final answer\n");
	// char buffer[256];
	// bzero(buffer, 256);
